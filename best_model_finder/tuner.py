...
class Model_Finder:
    ...
    def get_best_params_for_SVM(self, train_x, train_y):
        ...
        try:
            ...
            self.param_grid = {'C': [1, 10, 100, 1000, 10000],
                                    'gamma': [1, 0.1, 0.01, 0.001, 0.0001],
                                    'kernel': ['rbf']}
            #Creating an object of the Grid Search class
            self.grid = GridSearchCV(estimator=self.svclassifier, param_grid=self.param_grid, cv=10,  verbose=3)
            ...
            self.svclassifier = SVC(C=self.C, gamma=self.gamma,
                                              kernel=self.kernel)
            # training the mew model
            self.svclassifier.fit(train_x, train_y)
            self.logger_object.log(self.file_object,
                                   'SVC best params: '+str(self.grid.best_params_)+'. Exited the get_best_params_for_SVM method of the Model_Finder class')
            ...
            self.logger_object.log(self.file_object,
                                   'Exception occured in get_best_params_for_SVM method of the Model_Finder class. Exception message:  ' + str(
                                       e))
            self.logger_object.log(self.file_object,
                                   'SVC Parameter tuning  failed. Exited the get_best_params_for_SVM method of the Model_Finder class')
            raise Exception()
    ...
    def get_best_model(self,train_x,train_y,test_x,test_y):
        ...
        try:
            ...
            self.svclassifier = self.get_best_params_for_SVM(train_x, train_y)
            self.prediction_SVM = self.svclassifier.predict(test_x)  # prediction using the SVM Algorithm
            ...
            elif (self.svm_score <  self.xgboost_score):
                return 'XGBoost', self.xgboost
            elif(self.random_forest_score >  self.xgboost_score):
                return 'random_forest', self.random_forest
            elif(self.random_forest_score >  self.svm_score):
                return 'random_forest', self.random_forest
            elif (self.random_forest_score <  self.svm_score):
                return 'svclassifier', self.svclassifier
            elif (self.xgboost_score <  self.svm_score):
                return 'svclassifier', self.svclassifier
