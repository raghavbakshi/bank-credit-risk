...
class prediction:
    ...
    def predictionFromModel(self):
        ...
        try:
            ...
            data_getter = data_loader_prediction.Get_Data_Predictor(self.file_object, self.log_writer)
            data = data_getter.get_data()

            preprocessor = preprocessing.Preprocessor(self.file_object, self.log_writer)
            data = preprocessor.remove_columns(data, ['Unnamed: 21'])
            ...
            clusters = kmeans.predict(data.drop(['serial'], axis=1))  # drops the first column for cluster prediction
            data['clusters'] = clusters
            clusters = data['clusters'].unique()
            for i in clusters:
                cluster_data = data[data['clusters'] == i]
                serial_names = list(cluster_data['serial'])
